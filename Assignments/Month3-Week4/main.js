...
// Get the selected archetype from localStorage or default to "Elemental HERO"
...
const API_URL = `https://db.ygoprodeck.com/api/v7/cardinfo.php?archetype=${encodeURIComponent(selectedArchetype)}`;
...
// Load images for the current page into IndexedDB
...
async function openModal(cardId, cards = allCards) {
    ...
    if (card) {
        ...
        const db = await initializeDB();
        const cachedImage = await loadImageFromIndexedDB(db, imageUrl);
        modalImage.src = cachedImage ? URL.createObjectURL(cachedImage) : imageUrl;
...
loadMoreButton.addEventListener("click", () => {
    currentPage++;
    renderCards(currentPage, filteredCards, true);
    updatePaginationControls(currentPage, totalPages);
});
...
document.querySelectorAll('button, input[type="number"]').forEach(element => {
